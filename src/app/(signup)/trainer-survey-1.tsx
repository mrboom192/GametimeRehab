...
export default function TrainerSurvey1() {
  ...
  const handleSelect = (answer: string) => {
    ...
    const updatedAnswers = signupData.trainer_challenges
      ? signupData.trainer_challenges.includes(answer)
        ? signupData.trainer_challenges.filter((item) => item !== answer) // Remove if already selected
        : [...signupData.trainer_challenges, answer] // Add if not selected
      : [answer]; // Initialize array if it's undefined or empty

    updateSignupData("trainer_challenges", updatedAnswers); // Update the state with the selected answers
    if (error) {
      ...
      <View className="flex-1 flex flex-col self-stretch justify-between items-start mx-8 mt-8 mb-16">
        <View className="flex flex-col self-stretch gap-4">
          <Questionnaire
            ...
            state={signupData.trainer_challenges}
